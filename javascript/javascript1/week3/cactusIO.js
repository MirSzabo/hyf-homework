...
function showStatus(activities, timeLimit) {
  ...
  return `You have added ${
    activities.length
  } activities. They amount to ${minutesOfUsage} min. of usage`;
}
...
function longestActivity() {
  const durations = activities.map(activity => activity.duration); //new array only with duration
  //console.log(durations);
  const highestAmount = Math.max(...durations); //spread operator instead of for loop
  const highestActivity = activities.filter(
    activity => activity.duration === highestAmount
  );
