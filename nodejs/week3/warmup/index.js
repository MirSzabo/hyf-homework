...
app.get("/calculator/addition", function(req, res) {
  let values = [];
  const firstParam = values.push(req.param("firstParam"));
  const secondParam = values.push(req.param("secondParam"));
  const valuesInOneArray = values.reduce((sum, val) => sum.concat(val), []);
